      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-wf-border p-5">
            ...
            <div className="space-y-3 mb-4">
              ...
              <select value={newUserRole} onChange={(e) => setNewUserRole(e.target.value as Role)} className="w-full px-3 py-2 border border-wf-border rounded text-sm bg-white">
                <option value="marketer">Marketer</option>
                <option value="system_admin">System Admin</option>
              </select>
              <input value={newUserBrands} onChange={(e) => setNewUserBrands(e.target.value)} placeholder="brand 指派（逗號分隔）" className="w-full px-3 py-2 border border-wf-border rounded text-sm font-mono" />
              <button onClick={addUser} className="px-3 py-2 bg-wf-black text-white rounded text-sm hover:translate-x-1.5 transition-all">新增帳號指派</button>
            </div>
            <div className="space-y-2">
              {users.map((user) =>
            <div key={user.id} className="border border-wf-border rounded p-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-wf-border p-5">
            ...
            <div className="space-y-3">
              {budgets.map((row, idx) =>
            <div key={row.channel} className="border border-wf-border rounded p-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                    <label className="border border-wf-border rounded p-2">總預算<input type="number" value={row.total} onChange={(e) => setBudgets((prev) => prev.map((item, i) => i === idx ? { ...item, total: Number(e.target.value) || 0 } : item))} className="mt-1 w-full px-2 py-1 border border-wf-border rounded" /></label>
                    <label className="border border-wf-border rounded p-2">已用<input type="number" value={row.used} onChange={(e) => setBudgets((prev) => prev.map((item, i) => i === idx ? { ...item, used: Number(e.target.value) || 0 } : item))} className="mt-1 w-full px-2 py-1 border border-wf-border rounded" /></label>
                    <div className="border border-wf-border rounded p-2">使用率<div className="mt-1 font-mono text-sm">{Math.round(row.used / Math.max(1, row.total) * 100)}%</div></div>
                  </div>
          ...
          <div className="bg-white rounded-xl border border-wf-border p-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <label className="block">緊急門檻（%）<input type="number" value={budgetCritical} onChange={(e) => setBudgetCritical(Number(e.target.value) || 0)} className="mt-1 w-full px-3 py-2 border border-wf-border rounded" /></label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-wf-border p-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              <label className="flex items-center justify-between border border-wf-border rounded p-3"><span>預算達門檻告警</span><input type="checkbox" checked={notifyBudget} onChange={(e) => setNotifyBudget(e.target.checked)} /></label>
          ...
          <div className="bg-white rounded-xl border border-wf-border p-5">
            ...
            <div className="space-y-3">
              <label className="block text-sm">Slack Webhook（遮罩顯示）
                <input value={slackWebhookMasked} onChange={(e) => setSlackWebhookMasked(e.target.value)} className="mt-1 w-full px-3 py-2 border border-wf-border rounded text-sm font-mono" />
              </label>
              <label className="block text-sm">Email 收件群組
                <input defaultValue="cora-admin@tvbs.com.tw, martech@tvbs.com.tw" className="mt-1 w-full px-3 py-2 border border-wf-border rounded text-sm" />
              </label>
